...
export class SecondHandlerFunctionStack extends cdk.Stack {
  ...
  constructor(scope: cdk.Construct, id: string, props?: CommonProps) {
    ...
    if (props?.apiGatewayRestApi) {
      ...
      const lambdaRole = new iam.Role(this, 'FunctionRole', {
        ...
        roleName: `${appEnv}-second-handler-function-role`,
      });
      ...
      const lambdaFn = new lambda.Function(this, 'SecondHandlerFunction', {
        runtime: lambda.Runtime.NODEJS_12_X,
        code: lambda.Code.fromBucket(functionArtifactsBucket, `second-handler-function/${lambdaTag}`),
        handler: 'index.handler',
        functionName: `${appEnv}-second-handler-function`,
        role: lambdaRole,
      ...
      const apiResource = apigw.root.addResource('second');
      apiResource.addMethod('GET', new apigateway.LambdaIntegration(lambdaFn, { proxy: true }));
  
      new cdk.CfnOutput(this, 'SecondHandlerFunctionArn', {
        value: lambdaFn.functionArn,
        exportName: `SecondHandlerFunctionArn-${appEnv}`,
        description: 'Second handler function ARN',
      });
  
      new cdk.CfnOutput(this, 'SecondHandlerFunctionName', {
        value: lambdaFn.functionName,
        exportName: `SecondHandlerFunctionName-${appEnv}`,
        description: 'Second handler function Name',
      });
